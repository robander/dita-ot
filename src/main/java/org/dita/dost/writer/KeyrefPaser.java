        ...
        if (keyrefLevel != 0 && empty) {
            ...
            if (!validKeyref.isEmpty() && validKeyref.peek()) {
                ...
                if (elemName.peek().equals(MAP_TOPICREF.localName)) {
                    final NodeList topicmetaNodes = elem.getElementsByTagName(MAP_TOPICMETA.localName);
                    if (topicmetaNodes.getLength() > 0) {
                        domToSax((Element) topicmetaNodes.item(0), true, false);
                    }
